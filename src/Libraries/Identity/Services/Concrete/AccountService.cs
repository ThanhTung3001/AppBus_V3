...
namespace Identity.Services.Concrete
{
    public class AccountService : IAccountService
    {
        ...
        public async Task<BaseResponse<object>> AuthenticateWithEmailAsync(AuthenticationRequestEmail request)
        {

            var user = _userManager.Users.Where(e => e.Cccd_id == request.CccdId).FirstOrDefault();
            if (user != null)
            {
                throw new Exception("CCCD is exist, try another icccid");
            }
            user = _userManager.Users.Where(e => e.Email == request.Email).FirstOrDefault();
            if (user != null)
            {
                throw new Exception("Email is exist, try another email");
            }
            var authenCode = RandomCode(6);
            var createUser = _userManager.CreateAsync(new ApplicationUser()
            {
                Email = request.Email,
                UserName = request.CccdId,
                Cccd_id = request.CccdId,
                AuthCode = authenCode,
                CreateDateAuthCode = DateTime.Now,
            ...
            await _emailService.SendAsync(new EmailRequest()
            {
                From = "appbus.huecit@gmail.com",
                To = request.Email,
                Body = $"Mã xác thực của bạn là {authenCode}",
                Subject = "Mã xác thực"
            });
            return null;
        ...
        {
            // int codeLength = 6;
            byte[] randomNumber = new byte[codeLength];
            ...
            int codeValue = Math.Abs(BitConverter.ToInt32(randomNumber, 0));
            string authenticationCode = codeValue.ToString().Substring(0, codeLength);
            return authenticationCode;
        }

        

        public async Task<BaseResponse<AuthenticationResponse>> ValidationAuthCode(ValidationAuthCode request)
        {
            var user = _userManager.Users.Where(e => e.Cccd_id == request.ICCID).FirstOrDefault();
            if (user == null)
            {
                throw new Exception("CCCD is not exist, try another icccid");
            }
            else if (user.AuthCode != request.Code)
            {
                throw new Exception("Code not valid");
            }
            else if (DateTime.Now.Subtract(user.CreateDateAuthCode) > TimeSpan.FromMinutes(15))
            {
            ...
            await _userManager.UpdateAsync(user);
               ...
               return await AuthenticateAsync(new AuthenticationRequest(){
                    Email  = user.Email,
                    Password="Abcd@123"
                });
            // return null;
        ...
        public async Task<BaseResponse<object>> AuthenticateWithEmailGetCodeAsync(AuthenticationRequestEmail request)
        {
             var user = _userManager.Users.Where(e => e.Cccd_id == request.CccdId).FirstOrDefault();
            if (user == null)
            {
            ...
            var authenCode = RandomCode(6);
            user.AuthCode = authenCode;
            user.CreateDateAuthCode  = DateTime.Now;
            var createUser = _userManager.UpdateAsync(user);
            await _emailService.SendAsync(new EmailRequest()
            {
                From = "appbus.huecit@gmail.com",
                To = request.Email,
